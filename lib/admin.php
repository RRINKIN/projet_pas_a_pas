...
function getAdmin($id = null, $alpha = ""){
    ...
    if(is_null($id)){
        ...
        $sql = "SELECT admin_id, login, street, num, zip, city, pseudo, is_visible FROM admin ".$cond." ORDER BY login;";
    }else{
        if(is_numeric($id)){
            $sql = "SELECT login, street, num, zip, city, pseudo, level_access, password, is_visible FROM admin WHERE admin_id = $id;";
        }
    }
