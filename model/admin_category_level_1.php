...
switch($get_action){
    ...
    // dans ce cas-ci, on désire ajouter un category_level_1
    // deux cas de figure :
    ...
    case "add":
        ...
        */
        ...
        $input[] = addInput('Category_level_1', ["type" => "text", "value" => $post_category_level_1, "name" => "category_level_1", "class" => "u-full-width"], true, "twelve columns");
        $input[] = addLayout("</div>");
        ...
        if($show_form != false){
            ...
            $data_values                        = array();
            $data_values["category_level_1"]    = $post_category_level_1;
            // exécution de la requête
            if(insertCategory_level_1($data_values)){
    ...
    case "update":
        ...
        // récupération / initialisation des données qui transitent via le formulaire via la fonction init_tagname
        $array_name = [
            "category_level_1" => ["string", $category_level_1]
        ];
        // appel de la fonction qui est chargée d'initialiser et récupérer les données provenant du formulaire sur base du array $array_name
        init_tagname($array_name);
        ...
        */
        ...
        $input = [];
        // ajout des différents champs du formulaire
        $input[] = addLayout("<h4>Modifier une categorie level 1</h4>");
        $input[] = addLayout("<div class='row'>");
        $input[] = addInput('Category_level_1', ["type" => "text", "value" => $post_category_level_1, "name" => "category_level_1", "class" => "u-full-width"], true, "twelve columns");
        $input[] = addLayout("</div>");
        $input[] = addSubmit(["value" => "envoyer", "name" => "submit"], "\t\t<br />\n");
        ...
        if($show_form != false){
            ...
            if(updateCategory_level_1($get_category_level_1_id, $data_values)){
                // message de succes qui sera affiché dans le <body>
    ...
    case "showHide":
        ...
        if(showHideCategory_level_1($get_category_level_1_id)){
            // message de succes qui sera affiché dans le <body>
